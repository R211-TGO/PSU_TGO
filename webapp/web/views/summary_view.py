...
def get_stats():
    ...
    if selected_scopes:
        valid_sub_scopes = [
            scope
            for scope in all_selected_sub_scopes
...
def get_charts():
    ...
    # คำนวณข้อมูล emissions สำหรับปีที่เลือกและปีก่อนหน้า
    current_year_data = calculate_emissions_data(
        user, valid_sub_scope_ids, time_period, selected_year
    )
    previous_year_data = calculate_emissions_data(
        user, valid_sub_scope_ids, time_period, selected_year - 1
    )
...
def calculate_emissions_data(user, sub_scopes, time_period, selected_year=None):
    ...
    print(
        f"Found {last_year_materials.count()} materials for year {int(selected_year) - 1}"
    )
